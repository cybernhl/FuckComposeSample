    ...
    override suspend fun load(
        ...
        val loadKey = when (loadType) {
            ...
            LoadType.REFRESH -> {
                ...
                Log.i("ArticleRemoteMediator", "LoadType = REFRESH")
                0
            }

            LoadType.PREPEND -> {
                //在当前列表头部添加数据的时候使用
                Log.i("ArticleRemoteMediator", "LoadType = PREPEND")
                return MediatorResult.Success(endOfPaginationReached = true)
            }

            LoadType.APPEND -> {
                //在下拉加载更多数据的时候使用
                Log.i("ArticleRemoteMediator", "LoadType = APPEND")
//                wanAndroidDatabase.withTransaction {
//                    wanAndroidDatabase.curPageDao().getCurPage().curPage + 1
//                }
                mmkv.decodeInt("CurPageIndex", 0)
